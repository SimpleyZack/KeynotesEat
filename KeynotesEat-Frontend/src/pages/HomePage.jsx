    ...
    <div className="min-h-screen bg-[#F8F9FB] flex flex-col font-sans">
      ...
      <header className="h-20 bg-white border-b border-gray-100 flex items-center px-8 sticky top-0 z-30 gap-6 shadow-sm">
        <div className="flex items-center gap-3 w-52 shrink-0">
          <div className="w-10 h-10 bg-[#C17A3D] rounded-xl flex items-center justify-center text-white font-black text-lg shadow-md shadow-[#E0B18A]">K</div>
          <div>
            <h1 className="font-black text-gray-800 text-lg leading-none">Keynotes<span className="text-[#C17A3D]">Eats</span></h1>
            <p className="text-[10px] text-gray-400 font-medium tracking-wide mt-1">HIDDEN GEM RECIPES</p>
          </div>
        </div>
        <div className="flex flex-1 items-center gap-3 bg-gray-50/80 border border-gray-200 rounded-full px-5 h-12 max-w-2xl transition-all focus-within:bg-white focus-within:ring-2 focus-within:ring-[#F7D7B0] focus-within:border-[#D9985D]">
      ...
      <div className="flex flex-1">
        ...
        <aside className="w-64 hidden lg:flex flex-col py-8 px-5 sticky top-20 h-[calc(100vh-80px)] border-r border-gray-100 bg-white">
          ...
          <div className="bg-[#FFF1E0] border border-[#F7D7B0] rounded-3xl p-5 mt-auto">
            <div className="flex items-center gap-2 mb-3">
              <span className="bg-white p-1.5 rounded-lg shadow-sm">&#128081;</span>
              <span className="text-xs font-black text-[#A26033] tracking-wide uppercase">Chef tip</span>
            </div>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            {page === 'home' && !selectedRecipe && (
              <div className="animate-slide-up">
                ...
                <div className="bg-[#FFF5EB] rounded-[40px] p-12 lg:p-16 mb-12 relative overflow-hidden flex items-center min-h-[320px]">
                  <div className="relative z-10 max-w-2xl">
                    <div className="inline-flex items-center gap-2 bg-white/70 text-[#A26033] text-xs font-black px-4 py-2 rounded-full mb-6 tracking-wide shadow-sm">
                    ...
                    <h1 className="text-5xl lg:text-6xl font-black text-gray-900 leading-[1.1] mb-5 tracking-tight">
                      ...
                      <span className="text-[#C17A3D]">made simple at home.</span>
                    </h1>
                  ...
                  {/* Ornamen Banner */}
                  <div className="absolute -right-12 -top-12 w-80 h-80 bg-[#F9D5AB] rounded-full opacity-30 blur-3xl"></div>
                  <div className="absolute right-32 -bottom-20 w-64 h-64 bg-[#F6B16B] rounded-full opacity-25 blur-2xl"></div>
                </div>
                ...
                <div className="flex justify-between items-end mb-8">
                  ...
                  <button className="text-[#C17A3D] text-sm font-bold hover:text-[#A05F2C] transition-colors">View all &#8594;</button>
                </div>
            ...
            {page === 'home' && selectedRecipe && (
              <div className="max-w-5xl mx-auto animate-slide-up">
                <button
                  ...
                  className="text-gray-500 text-sm mb-8 hover:text-[#C17A3D] transition-colors flex items-center gap-2 font-bold bg-white px-4 py-2 rounded-full border border-gray-200 shadow-sm w-fit"
                >
                  &#8592; Kembali ke semua resep
                </button>

                {/* Hero Detail */}
                <div className="bg-[#FFF5EB] rounded-[40px] p-8 lg:p-12 flex flex-col md:flex-row gap-10 items-center mb-10 relative overflow-hidden">
                  <div className="flex-1 z-10">
                    <div className="inline-flex items-center gap-1.5 bg-white/70 text-[#A26033] text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest mb-5">
                    ...
                    <div className="flex gap-3 flex-wrap">
                      <span className="bg-white px-5 py-2.5 rounded-full text-sm font-bold text-gray-700 shadow-sm flex items-center gap-2">
                        <span className="text-[#C17A3D]">&#9201;</span> {selectedRecipe.waktu} min
                      </span>
                      <span className="bg-white px-5 py-2.5 rounded-full text-sm font-bold text-gray-700 shadow-sm flex items-center gap-2">
                        <span className="text-[#C17A3D]">&#128202;</span> {selectedRecipe.level}
                      </span>
                    </div>
                  </div>
                  <div className="md:w-[45%] w-full h-80 z-10">
                   ...
                   {/* Ornamen */}
                   <div className="absolute left-0 bottom-0 w-full h-1/2 bg-gradient-to-t from-[#F7D7B0]/50 to-transparent pointer-events-none"></div>
                </div>
                    ...
                    <div className="space-y-4">
                      {JSON.parse(selectedRecipe.ingredients || '[]').map((item, index) => (
                        <div key={index} className="flex items-center gap-4 py-3 border-b border-gray-100 last:border-0">
                          <span className="text-[#C9A06C] font-black text-lg w-6 shrink-0">
                    ...
                    <div className="space-y-6">
                      {JSON.parse(selectedRecipe.steps || '[]').map((step, index) => (
                        <div key={index} className="flex gap-5">
                          <div className="w-10 h-10 bg-[#F9D5AB] rounded-2xl flex items-center justify-center text-[#A26033] font-black shrink-0 text-sm mt-1">
                            {index + 1}
                          </div>
            ...
            {page === 'add' && (
              <div className="max-w-6xl mx-auto animate-slide-up">
                ...
                {/* Banner Add */}
                <div className="bg-[#FFF7F0] rounded-[28px] p-8 lg:p-10 mb-8 relative overflow-hidden flex flex-col lg:flex-row items-start gap-8">
                  <div className="relative z-10 max-w-full lg:max-w-2xl">
                    <div className="inline-flex items-center gap-2 bg-white/75 text-[#A26033] text-[11px] font-black px-4 py-2 rounded-full mb-4 tracking-wide shadow-sm">
                      &#128203; Share your kitchen secret
                    </div>
                    <h2 className="text-3xl lg:text-4xl font-black text-gray-900 leading-snug tracking-tight">
                      Add a new recipe,
                      <br />
                      <span className="text-[#C17A3D]">make someone's day better.</span>
                    </h2>
                  ...
                  <div className="absolute -right-8 -top-8 w-60 h-60 bg-[#F9D5AB] rounded-full opacity-25 blur-3xl"></div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="lg:col-span-2 space-y-8">
                      ...
                      <div className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-2">
                            ...
                            <input name="title" placeholder="e.g. Miso Butter Pasta" className="w-full bg-gray-50/50 border border-gray-200 rounded-2xl p-4 text-sm font-medium focus:outline-none focus:bg-white focus:border-[#D9985D] focus:ring-4 focus:ring-[#F8E5D0] transition-all" onChange={handleChange} required />
                          </div>
                          <div className="space-y-2">
                            <label className="text-xs font-black text-gray-500 tracking-wider">NAMA MENU</label>
                            <input name="name" placeholder="e.g. Pasta" className="w-full bg-gray-50/50 border border-gray-200 rounded-2xl p-4 text-sm font-medium focus:outline-none focus:bg-white focus:border-[#D9985D] focus:ring-4 focus:ring-[#F8E5D0] transition-all" onChange={handleChange} required />
                          </div>
                        </div>

                        <div className="space-y-2">
                          <label className="text-xs font-black text-gray-500 tracking-wider">SHORT DESCRIPTION</label>
                          <textarea name="description" placeholder="A creamy, umami-packed pasta ready in 20 minutes..." className="w-full bg-gray-50/50 border border-gray-200 rounded-2xl p-4 text-sm font-medium h-24 focus:outline-none focus:bg-white focus:border-[#D9985D] focus:ring-4 focus:ring-[#F8E5D0] transition-all resize-none" onChange={handleChange}></textarea>
                        </div>

                        <div className="grid grid-cols-2 gap-6">
                          <div className="space-y-2">
                            <label className="text-xs font-black text-gray-500 tracking-wider">COOK TIME (MINUTES)</label>
                            <input name="waktu" type="number" placeholder="20" className="w-full bg-gray-50/50 border border-gray-200 rounded-2xl p-4 text-sm font-medium focus:outline-none focus:bg-white focus:border-[#D9985D] focus:ring-4 focus:ring-[#F8E5D0] transition-all" onChange={handleChange} required />
                          </div>
                          <div className="space-y-2">
                            <label className="text-xs font-black text-gray-500 tracking-wider">DIFFICULTY</label>
                            <select name="level" className="w-full bg-gray-50/50 border border-gray-200 rounded-2xl p-4 text-sm font-medium focus:outline-none focus:bg-white focus:border-[#D9985D] focus:ring-4 focus:ring-[#F8E5D0] transition-all cursor-pointer" onChange={handleChange}>
                    ...
                    <div className="bg-white rounded-[28px] p-8 lg:p-10 border border-gray-100 shadow-sm">
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-2">
                        ...
                        <button type="button" onClick={() => handleAddField('ingredients')} className="text-xs font-bold text-[#C17A3D] bg-[#FFF1E0] px-3 py-1.5 rounded-full hover:bg-[#F9D3A9] transition-colors flex items-center gap-1">
                          + Add
                        </button>
                      </div>
                      ...
                      <div className="space-y-4">
                        {formData.ingredients.map((item, index) => (
                          <div key={`ingredient-${index}`} className="flex flex-col gap-3 sm:flex-row sm:items-center bg-gray-50/80 border border-gray-100 rounded-2xl p-3">
                            <span className="text-[#C9A06C] font-black text-sm w-6 text-right shrink-0">
                              {(index + 1).toString().padStart(2, '0')}
                            </span>
                            <input
                              type="text"
                              ...
                              value={item}
                              className="flex-1 min-w-0 bg-white border border-gray-200 rounded-xl p-3 text-sm font-medium focus:outline-none focus:border-[#D9985D] focus:ring-2 focus:ring-[#F8E5D0] transition-all"
                    ...
                    <div className="bg-white rounded-[28px] p-8 lg:p-10 border border-gray-100 shadow-sm">
                       <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-2">
                        ...
                        <button type="button" onClick={() => handleAddField('steps')} className="text-xs font-bold text-[#C17A3D] bg-[#FFF1E0] px-3 py-1.5 rounded-full hover:bg-[#F9D3A9] transition-colors flex items-center gap-1">
                          + Add step
                        </button>
                      </div>
                      ...
                      <div className="space-y-4">
                        {formData.steps.map((item, index) => (
                          <div key={`step-${index}`} className="flex flex-col gap-4 sm:flex-row sm:items-start bg-gray-50/80 border border-gray-100 rounded-2xl p-4">
                            <div className="w-8 h-8 bg-[#C17A3D] rounded-full flex items-center justify-center text-white font-black text-xs shrink-0 mt-1 shadow-md shadow-[#E0B18A]">
                              {index + 1}
                            </div>
                            <textarea
                              placeholder={`Describe what to do in this step...`}
                              value={item}
                              className="flex-1 min-w-0 bg-white border border-gray-200 rounded-xl p-4 text-sm font-medium min-h-[100px] focus:outline-none focus:border-[#D9985D] focus:ring-2 focus:ring-[#F8E5D0] transition-all resize-none"
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="bg-white rounded-[32px] p-8 border border-gray-100 shadow-sm">
                      ...
                      <div className="bg-[#FFF6F0] border-2 border-dashed border-[#F7D7B0] rounded-3xl p-6 text-center transition-all hover:bg-[#FFF3E3] mb-6">
                        {/* Pilihan Upload/URL */}
                        <div className="flex justify-center gap-6 mb-6">
                          <label className="flex items-center gap-2 text-sm font-bold text-gray-600 cursor-pointer">
                            <input type="radio" name="imageMode" value="url" checked={imageMode === 'url'} onChange={() => {
                              ...
                              setImageFile(null);
                              setPreviewImage(null);
                            }} className="accent-[#C17A3D] w-4 h-4 scale-125" />
                            URL Link
                          </label>
                          <label className="flex items-center gap-2 text-sm font-bold text-gray-600 cursor-pointer">
                            <input type="radio" name="imageMode" value="upload" checked={imageMode === 'upload'} onChange={() => setImageMode('upload')} className="accent-[#C17A3D] w-4 h-4 scale-125" />
                            Upload
                          </label>
                        </div>
                        
                        {imageMode === 'url' ? (
                          <input name="image_url" placeholder="Paste link gambar..." value={formData.image_url} className="w-full bg-white rounded-xl p-3 text-sm font-medium focus:outline-none focus:border-[#D9985D] border border-[#F7D7B0] shadow-sm text-center" onChange={handleChange} />
                        ) : (
                          <div className="relative">
                            ...
                            />
                            <div className="bg-white py-4 rounded-xl border border-[#F7D7B0] shadow-sm text-[#A26033] font-bold text-sm">
                      ...
                      <div className="bg-[#FFF1E0] rounded-2xl p-4">
                        <div className="flex items-center gap-2 mb-2">
                           <span className="text-[#C17A3D]">&#128081;</span>
                           <span className="text-xs font-black text-[#A26033]">Pro tip</span>
                        </div>
                        <p className="text-xs text-gray-500 font-medium leading-relaxed">
      ...
      <nav className="lg:hidden fixed bottom-6 left-6 right-6 bg-white/90 backdrop-blur-xl border border-gray-200 h-16 rounded-2xl shadow-2xl flex justify-around items-center z-40 px-4">
        <button onClick={() => { setPage('home'); setSelectedRecipe(null); }} className={`p-2.5 rounded-xl transition-colors ${page === 'home' ? 'text-[#C17A3D] bg-[#FFF3E0]' : 'text-gray-400 hover:bg-gray-50'}`}>&#127968;</button>
        <button onClick={() => setPage('add')} className={`p-2.5 rounded-xl transition-colors ${page === 'add' ? 'text-[#C17A3D] bg-[#FFF3E0]' : 'text-gray-400 hover:bg-gray-50'}`}>&#43;</button>
        <button onClick={() => setPage('saved')} className={`p-2.5 rounded-xl transition-colors ${page === 'saved' ? 'text-[#C17A3D] bg-[#FFF3E0]' : 'text-gray-400 hover:bg-gray-50'}`}>&#128278;</button>
        <button onClick={() => setPage('about')} className={`p-2.5 rounded-xl transition-colors ${page === 'about' ? 'text-[#C17A3D] bg-[#FFF3E0]' : 'text-gray-400 hover:bg-gray-50'}`}>&#128100;</button>
      </nav>
    </div>
  );
}
...
function SidebarItem({ icon, label, active, onClick }) {
  return (
    <div onClick={onClick} className={`flex items-center gap-4 px-4 py-3.5 rounded-2xl cursor-pointer transition-all duration-300 ${active ? 'bg-[#FFF3E0] text-[#A26033]' : 'hover:bg-gray-50 text-gray-500 hover:text-gray-900'}`}>
  ...
  return (
    <div onClick={onClick} className="group bg-white rounded-[28px] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-300 border border-gray-100 cursor-pointer flex flex-col h-full active:scale-[0.98]">
      ...
      <div className="p-5 flex-1 flex flex-col justify-center">
        <h3 className="font-black text-gray-900 text-lg group-hover:text-[#C17A3D] transition-colors line-clamp-1 mb-1 tracking-tight">{nama}</h3>
        <p className="text-sm font-medium text-gray-400 flex items-center gap-1.5">
          <span className="text-[#C9A06C]">&#9201;</span> {waktu}
        </p>
      </div>
    </div>
  );
